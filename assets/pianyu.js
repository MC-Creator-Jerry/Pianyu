...
window.PY = {
  ...
  card(v) {
    ...
    const wt = v.workType;
    const wtTxt = wt ? ((window.PY.WORK_TYPES && window.PY.WORK_TYPES[wt]) || wt) : '';
    const tags = (v.tags || [])
    ...
    a.innerHTML = `
      ...
      <div class="body">
        <div class="title">${window.PY.esc(v.title)}${wt ? ` <span class="wt-badge wt-${wt}">${wtTxt}</span>` : ''}</div>
...
PY.user = {
  ...
  async mount(slotId) {
    ...
    wrap.innerHTML =
      ...
      '<div class="sep"></div>' +
      '<a role="menuitem" href="creator.html">创作岛</a>' +
